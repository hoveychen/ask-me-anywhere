...
| 方法 | 路径 | Auth | 入参 | 返回 |
|---|---|---|---|---|
| GET | `/healthz` | 公开 | — | `200 ok` |
| POST | `/push` | Bearer | [CardInput JSON](#cardinput-json) | `200 {"id":"<uuid>"}` 或 `401` / `422` |
| POST | `/github/pr` | Bearer | GitHub `pull_request` webhook payload | `200 {"id":"<uuid>","ignored":false}` 或 `200 {"id":null,"ignored":true}` (非 actionable action) / `401` / `422` |

`/healthz` 不走 auth,给 Traefik / Muvee / k8s liveness probe 用。其他路由走中间件:无 token → `401 unauthorized`,token 不匹配 → 同样 401。JSON body 解析失败 → `422 Unprocessable Entity`(axum 默认行为)。
...
| `a2ui` | object | 否 | `{}` | A2UI message tree。空对象时 Flutter 端就显示一个纯摘要卡片。详见 §4 |
