...
|---|---|---|---|---|
| GET | `/healthz` | open | — | `200 ok` |
| POST | `/push` | Bearer | [CardInput JSON](#cardinput-json) | `200 {"id":"<uuid>"}` or `401` / `422` |
| POST | `/github/pr` | Bearer | GitHub `pull_request` webhook payload | `200 {"id":"<uuid>","ignored":false}` or `200 {"id":null,"ignored":true}` (non-actionable action) / `401` / `422` |

`/healthz` is unauthed — point Traefik / Muvee / k8s liveness probes here. Other routes go through middleware: no token → `401 unauthorized`, wrong token → also 401. JSON parse failure → `422 Unprocessable Entity` (axum default).
...
## 3. CardInput JSON {#cardinput-json}
...
| `a2ui` | object | no | `{}` | A2UI message tree. With `{}`, the Flutter end shows a summary-only card. See §4. |
...
### 4.4 Form input (two-way data-model sync)
...
card = {
    "summary": "DB backup completed",
    "a2ui": {
        "root": {
            "Card": {
                "id": "root",
                "children": [
                    {"Text": {"id": "ok", "text": "Backup size: 4.7 GB"}},
                ],
            }
        }
    },
